...
public abstract class AbstractRecommenderSimilarity implements RecommenderSimilarity {
    ...
    public void buildSimilarityMatrix(DataModel dataModel) {
        ...
        String similarityKey = conf.get("rec.recommender.similarity.key", "item");
        if(StringUtils.isNotBlank(similarityKey)){
            if (StringUtils.equals(similarityKey, "social")) {
                ...
                System.out.println("similarityKey: "+similarityKey);
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    for (int j = i + 1; j < count; j++) {
                        ...
                        if (!Double.isNaN(sim)) {
                            similarityMatrix.set(i, j, sim);// matrix de similaridade entre usuarios ou itens, por xemplo usuario 0 com i=suario 1
                        }
                    }
                }
            }
    ...
    public double getCorrelation(SparseVector thisVector, SparseVector thatVector) {
        ...
        double sim = getSimilarity(thisList, thatList);// similaridade cosseno
