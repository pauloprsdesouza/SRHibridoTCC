...
public class ItemKNNRecommender extends AbstractRecommender {
    ...
    public double predict(int userIdx, int itemIdx) throws LibrecException {
        ...
        if (!(null != itemSimilarityList && itemSimilarityList.length > 0)) {
            createItemSimilarityList(); System.out.println("criou: ");
        }
     ...
     */
    public void createItemSimilarityList() {// a cada interaçã eçe pega a linha de item imilares e joga em iteemsimilaritylist, sendo que a cada poseoção de iteemsimilaritylist contem uma lista como id o utem similar e o vlor de sim
