...
public abstract class AbstractRecommender implements Recommender {
    ...
    protected void setup() throws LibrecException {
        ...
        numUsers = trainMatrix.numRows(); System.out.println("numUsers: "+numUsers);
...
public void getItensSimilar() {
   
	List<RecommendedItem> recommendedItemList = this.getRecommendedList();
	SymmMatrix similarityMatrix = context.getSimilarity().getSimilarityMatrix();
	
	for (RecommendedItem recommendedItem : recommendedItemList) {
	int itemIdx = Integer.parseInt(recommendedItem.getItemId());
    ...
    while (simItr.hasNext()) {
        ...
        if(simVectorEntry.get() > 0) {
        System.out.println("item: "+itemIdx+" similarItem: "+simVectorEntry.index()+" valor: "+simVectorEntry.get());
        }
    }
	}
    }
