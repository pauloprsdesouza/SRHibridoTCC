...
public class SparseMatrix implements Iterable<MatrixEntry>, DataMatrix, Serializable {
    ...
    public double get(int row, int column) {
    	//System.out.println("rowPtr[row]: "+rowPtr[row]); 
    ...
    public Set<Integer> getColumnsSet(int row) {
      ...
      //  System.out.println("rowPtr[row]: "+rowPtr[row]);
        if (row < numRows) {//System.out.println("rowPtr[row + 1]: "+rowPtr[row + 1]);
            for (int j = rowPtr[row]; j < rowPtr[row + 1]; j++) {//System.out.println("j: "+j); 
                int col = colInd[j];
                double val = rowData[j];
                if (val >= 4.0)
                    res.add(col);
